...
@app.get("/leads/quick")
def get_leads_quick():
    """Fast endpoint — returns leads immediately with no email enrichment."""
...
def enrich_one_lead(website: str = "", domain: str = "", company: str = ""):
    """
    Enrich a SINGLE lead with email on demand.
    Call this when user clicks Email button on a specific lead.
    Much faster than enriching all leads at once.
    """
    # Build a minimal lead dict
    lead = {
        "business": company,
        "website": website,
        "email": ""
...
def send_via_resend(to_email: str, subject: str, body: str) -> dict:
    """Send email through Resend. Returns dict with status and error detail."""
    if not RESEND_API_KEY:
        return {"ok": False, "error": "RESEND_API_KEY not set in Railway variables"}
    if not FROM_EMAIL:
        return {"ok": False, "error": "FROM_EMAIL not set in Railway variables"}
    try:
        res = requests.post(
        ...
        if res.status_code in (200, 201):
            return {"ok": True, "id": res.json().get("id", "")}
        else:
            # Return Resend's actual error message
            try:
                detail = res.json()
            except Exception:
                detail = res.text
            return {"ok": False, "error": f"Resend {res.status_code}: {detail}"}
...
def cors_json(status_code: int, content: dict):
    """Return a JSONResponse that always includes CORS headers."""
    return JSONResponse(
...
def generate_email(req: EmailRequest):
    ...
    if not req.email or "@" not in req.email:
        return cors_json(400, {"error": "Valid recipient email address required"})

    # Generate email body with Claude
    try:
        result = generate_email_with_claude(req)
    except Exception as e:
        return cors_json(500, {"error": f"Email generation failed: {str(e)}"})
    ...
    # Send via Resend
    send_result = send_via_resend(req.email, result["subject"], result["body"])
...
@app.options("/email/generate")
@app.options("/email/send")
async def email_options():
    """Handle CORS preflight requests for email endpoints."""
    return cors_json(200, {})
